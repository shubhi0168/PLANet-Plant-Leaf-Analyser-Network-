...
os.getcwd()
os.listdir()
path1 = os.path.join(os.getcwd(),'plant_disease_dataset')
os.listdir(path1)
path2 = os.path.join(path1,'PlantVillage/Tomato_healthy')
...
class DCGAN:
    ...
    def build_generator(self):
        ...
        model = Sequential()
        model.add(layers.Dense(8*8*1024, use_bias=False, input_shape=(300,)))
    ...
    def gene_imgs(self, count):
        ...
        noise = np.random.normal(0, 1, (count, 300))
...
s = np.array(Image.open(os.path.join(d,'1500.png')))
print(s.shape)
plt.imshow(s)
